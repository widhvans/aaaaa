                ...
                if batch_key_to_use in self.open_batches[user_id]:
                        ...
                        if dashboard_msg:
                            try:
                               await self.send_with_protection(
                                   ...
                                   f"⏳ **Status:** Added new file. Waiting 30 more seconds..."
                               )
                               self.last_dashboard_edit[edit_key] = time.time()
                            except MessageNotModified: pass
                    
                    batch_data['timer'] = loop.call_later(30, lambda u=user_id, k=batch_key_to_use: asyncio.create_task(self._finalize_batch(u, k)))
                else:
                    dashboard_msg = await self.send_with_protection(
                        ...
                        text=f"**New Batch Detected**\n\n"
                             f"🎬 **Batch:** `{batch_key_to_use}`\n"
                             f"📊 **Files Collected:** `1`\n"
                             f"⏳ **Status:** Collecting files. Waiting for 30 seconds..."
                    )
                    
                    self.open_batches[user_id][batch_key_to_use] = {
                        'messages': [copied_message],
                        'timer': loop.call_later(30, lambda u=user_id, k=batch_key_to_use: asyncio.create_task(self._finalize_batch(u, k))),
